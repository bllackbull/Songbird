...
function SemiCircleGauge({ pct, color = "emerald", label, sublabel, size = 120 }) {
  ...
  return (
    <div className="flex flex-col items-center" style={{ width: size }}>
      ...
      {sublabel && <p className="text-center text-[10px] text-slate-400 dark:text-slate-500">{sublabel}</p>}
    </div>
  );
}
...
export default function DashboardTab({ stats, onStatsChange }) {
  ...
  const infoCards = [
    { label: "Uploads",     value: sys ? fmtBytes(uploadsSize) : "—",     icon: Database,    hint: "Total size of uploaded files on disk" },
    { label: "Process RSS", value: sys ? fmtBytes(sys.memory.rss) : "—",  icon: MemoryStick, hint: "Total RAM used by the server process (Resident Set Size)" },
        ...
        <div className={cardCls + " p-4"}>
          {!sys ? (
            ...
            <div className="flex flex-wrap items-start justify-around gap-2">
              <SemiCircleGauge pct={loadPct} color={gaugeColor(loadPct)} label="CPU Load" sublabel={`${load1?.toFixed(2)} avg · ${sys.cpuCount} core${sys.cpuCount !== 1 ? "s" : ""}`} />
              <SemiCircleGauge pct={heapPct} color={gaugeColor(heapPct)} label="App Memory" sublabel={`${fmtBytes(sys.memory.heapUsed)} / ${fmtBytes(sys.memory.heapTotal)}`} />
              <SemiCircleGauge pct={sysPct} color={gaugeColor(sysPct)} label="System Memory" sublabel={`${fmtBytes(sys.memory.systemUsed)} / ${fmtBytes(sys.memory.systemTotal)}`} />
              <SemiCircleGauge pct={diskPct} color={gaugeColor(diskPct)} label="Disk Storage" sublabel={diskTotal > 0 ? `${fmtBytes(diskUsed)} / ${fmtBytes(diskTotal)}` : "Unavailable"} />
            </div>
          )}
        </div>
