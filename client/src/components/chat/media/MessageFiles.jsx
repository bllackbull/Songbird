...
const buildFallbackWaveform = (seed, count = 20) => {
  const seedValue = Array.from(String(seed || "audio")).reduce(
...
const VoiceMessageChip = memo(
  ({ file }) => {
    const serverUrl =
      file?.url && !String(file.url).startsWith("blob:")
        ? String(file.url)
        : "";
    ...
    }, [fileDuration, duration, setDuration]);
    ...
    const startProgressLoop = (audio) => {
      ...
      const tick = () => {
        ...
        if (
          ...
          window.localStorage?.getItem("sb-debug-voice") === "1"
        ) {
          setDebugInfo({
      ...
      <div className="inline-flex w-max max-w-full items-center gap-2 rounded-xl border border-emerald-200/70 bg-white px-3 py-2 text-xs text-slate-900 transition hover:border-emerald-300 hover:bg-white/90 hover:shadow-[0_0_16px_rgba(16,185,129,0.18)] dark:border-emerald-500/30 dark:bg-slate-900/50 dark:text-slate-200 dark:hover:bg-slate-900/70 dark:hover:shadow-[0_0_16px_rgba(16,185,129,0.14)]">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div
            ...
            style={{ height: `${waveformMaxHeight}px` }}
          >
            {peaks.map((height, idx) => {
              const barHeight = Math.round(
                6 + height * (waveformMaxHeight - 6),
              );
...
export function MessageFiles({
  ...
  const getFileNameParts = (name, maxBaseChars = 14) => {
    ...
    if (lastDot <= 0 || lastDot === fullName.length - 1) {
      const base =
        fullName.length > maxBaseChars + 3
            ...
            >
              <div className={imageFrameClass} style={mediaFrameStyle}>
                <img
                  ...
                  fetchPriority={
                    !isDesktop && fileIndex === 0 ? "high" : "auto"
                  }
        ...
        return file.url ? (
          ...
          >
            <span className="relative inline-flex h-5 w-5 shrink-0 items-center justify-center">
              <File
                size={18}
                className="absolute text-emerald-600 transition-opacity duration-150 group-hover:opacity-0 dark:text-emerald-300"
              />
              <Download
                size={18}
                className="absolute text-emerald-600 opacity-0 transition-opacity duration-150 group-hover:opacity-100 dark:text-emerald-300"
              />
            </span>
