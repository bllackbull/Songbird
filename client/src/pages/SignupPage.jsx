...
export default function SignupPage({ isDark, onToggleTheme, onSignup, onGoLogin, status }) {
  return (
    <section className="w-full max-w-md rounded-3xl border border-white/60 bg-white/80 p-6 shadow-2xl shadow-emerald-500/10 backdrop-blur dark:border-white/5 dark:bg-slate-900/80 sm:p-8">
      <div className="relative text-center">
        ...
        <h1 className="mt-2 font-display text-2xl font-bold sm:mt-3 sm:text-3xl">Join the flock</h1>
      ...
      <form className="mt-4 space-y-3 sm:mt-6 sm:space-y-4" onSubmit={onSignup}>
        <label className="block">
          <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 sm:text-sm">Nickname</span>
          <input
            name="nickname"
            type="text"
            required
        ...
        <label className="block">
          <span className="text-xs font-semibold text-slate-700 dark:text-slate-200 sm:text-sm">Confirm password</span>
          <input
