import { updateEnvValue } from "./secrets.js";
...
export function ensureValidVapidKeys({ projectRootDir, fs, path, webpush }) {
  ...
  try {
    updateEnvValue(envPath, "VAPID_PUBLIC_KEY", publicKey, { fsImpl: fs });
    updateEnvValue(envPath, "VAPID_PRIVATE_KEY", privateKey, { fsImpl: fs });
    if (!String(process.env.VAPID_SUBJECT || "").trim()) {
      updateEnvValue(envPath, "VAPID_SUBJECT", subject, { fsImpl: fs });
    }
  } catch (error) {
    console.warn(
