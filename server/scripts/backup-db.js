...
async function main() {
  ...
  const password =
    String(passwordFlag || '').trim() || (await promptSecret({ prompt: 'Backup password: ', required: true }))
  ...
  try {
    ...
    if (fs.existsSync(uploadsDir)) {
      fs.cpSync(uploadsDir, path.join(tempDir, 'data', 'uploads'), { recursive: true })
    }

    execFileSync(zipBinary, ['-r', '-P', password, backupPath, '.env', 'data'], {
      cwd: tempDir,
      stdio: 'pipe',
    })
  } catch (error) {
