...
describe("openDatabase Admin Utility", () => {
  ...
  test("creates SQLite DATA_DIR when openDatabase runs instead of on import", () => {
    ...
    const dbAdminUrl = pathToFileURL(
      path.resolve(path.dirname(new URL(import.meta.url).pathname), "../../scripts/_db-admin.js"),
    ).href;
    const script = `
      import fs from "node:fs";
      import path from "node:path";
    ...
    try {
      ...
      expect(result).toEqual({
        createdOnImport: false,
        sqliteExistsOnImport: false,
        createdOnOpen: true,
        sqliteExistsOnOpen: true,
      });
    } finally {
  ...
  test("uses the PostgreSQL knex client for SELECT 1 without creating songbird.db", async () => {
    ...
    const createKnexInstance = vi
      .spyOn(knexModule, "createKnexInstance")
      .mockReturnValue(mockKnex);
  ...
  test("opens PostgreSQL mode without creating its default SQLite database path", () => {
    ...
    const script = `
      ...
      const db = await openDatabase({ skipMigrations: true });
      const result = {
        createdDataDir: fs.existsSync(dataDir),
        createdSqliteFile: fs.existsSync(path.join(dataDir, "songbird.db")),
      };
      await db.close();
    ...
    fs.writeFileSync(
      loaderPath,
      `const mockKnexUrl = ${JSON.stringify(pathToFileURL(mockKnexPath).href)};
      export async function resolve(specifier, context, nextResolve) {
        const result = await nextResolve(specifier, context);
        if (result.url.endsWith("/server/db/knex.js")) {
          return { url: mockKnexUrl, shortCircuit: true };
        }
        return result;
      }
      `,
    ...
    try {
      const result = JSON.parse(
        execFileSync(
          process.execPath,
          ["--experimental-loader", loaderPath, "--input-type=module", "--eval", script],
          {
          encoding: "utf8",
          env: { ...process.env, DATA_DIR: dataDir, DB_CLIENT: "postgres" },
          },
        ),
      );
