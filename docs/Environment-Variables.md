...
| `STORAGE_PROCESSING_MODE` | `string` | `auto` | Media processing workflow mode (`auto`, `local`, `remote`). In `auto` (default), spawns the local worker and attempts to call the remote worker, retrying failed requests until `STORAGE_PROCESSING_TIMEOUT_MS` expires before falling back to the local worker. In `local`, dispatches directly and exclusively to the local worker. In `remote`, dispatches exclusively to the remote worker with retries up to `STORAGE_PROCESSING_TIMEOUT_MS` without running a local worker or falling back. |
