...
| `STORAGE_PROCESSING_MODE` | `string` | `auto` | Media processing workflow mode (`auto`, `local`, `remote`). In `auto` (default), runs the local worker and calls the remote worker with 3 retries before local fallback. In `local`, calls only the local worker. In `remote`, calls only the remote worker without running local workers or falling back. |
| `STORAGE_PROCESSING_TIMEOUT_MS` | `integer` | `30000` | Fallback timeout (ms) before local in-process transcoding takes over in `auto` mode. |
