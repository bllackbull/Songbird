...
| حالت | عملکرد |
|---|---|
| `auto` (پیشفرض) | **ارسال به ورکر ریموت با تلاش مجدد و بازگشت به ورکر محلی.** ورکر مدیا محلی در کانتینر اجرا میشود. ابتدا درخواست ترنسکد به ورکر ریموت (`WORKER_URL`) ارسال میشود و در صورت بروز خطا تا سقف `STORAGE_PROCESSING_TIMEOUT_MS` تلاش مجدد صورت میگیرد؛ در صورت عدم موفقیت یا عدم تنظیم ورکر ریموت، پردازش به ورکر محلی (`http://127.0.0.1:WORKER_PORT`) واگذار میگردد. |
| `local` | **پردازش انحصاری در ورکر محلی.** ورکر محلی در کانتینر اجرا شده و کارها فقط به ورکر محلی (`http://127.0.0.1:WORKER_PORT`) فرستاده میشوند و ورکر ریموت نادیده گرفته میشود. |
| `remote` | **پردازش انحصاری در ورکر ریموت.** ورکر محلی در کانتینر اجرا نمیشود و وظایف فقط به ورکر ریموت (`WORKER_URL`) ارسال میشوند و تا سقف `STORAGE_PROCESSING_TIMEOUT_MS` تلاش مجدد صورت میگیرد بدون هیچ بازگشتی به ورکر محلی. |
