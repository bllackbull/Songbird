...
1. **درخواست Presign**:
   - مرورگر کلاینت متادیتا فایل (`filename`، `mimeType`، `fileSize` و غیره) را به بک اند Songbird ارسال میکند.
   - سرور احراز هویت را اعتبارسنجی میکند، محدودیت حجم فایل (`FILE_UPLOAD_MAX_SIZE_MB`) را بررسی میکند، یک کلید ذخیره سازی یکتا میسازد و یک پالیسی موقت Presigned **POST** تولید میکند. (در صورت در دسترس نبودن امضای POST، به Presigned URL از نوع `PUT` برمیگردد.)
   - سرور یک رکورد آپلود معلق در جدول `pending_presigned_uploads` دیتابیس ثبت میکند.
2. **آپلود مستقیم مرورگر**:
   - مرورگر کلاینت باره (payload) فایل را مستقیماً به اندپوینت ذخیره سازی Cloudflare R2 / S3 آپلود میکند.
3. **تکمیل آپلود**:
   - پس از موفقیت آمیز بودن آپلود، کلاینت به Songbird اطلاع میدهد یا فایل را به درخواست پیام متصل میکند.
...
از آنجا که مرورگر کلاینت فایل ها را مستقیماً با استفاده از آپلود به اندپوینت Cloudflare R2 یا S3 آپلود میکند، **تنظیم CORS (Cross-Origin Resource Sharing) روی باکت الزامی است**. بدون تنظیم صحیح CORS، مرورگر درخواست های آپلود را به دلیل خطای Cross-Origin مسدود خواهد کرد.
