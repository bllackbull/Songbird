...
- مقدار `client_max_body_size` را در صورت استفاده از ذخیره سازی محلی (`STORAGE_DRIVER=local`) با `FILE_UPLOAD_MAX_TOTAL_SIZE_MB` همسو نگه دارید. 

:::

:::tip 

 در صورت فعال بودن `STORAGE_DRIVER=remote`، آپلودها مستقیماً با Presigned URL از مرورگر به Cloudflare R2 / S3 ارسال میشوند و محدودیتهای حجم Nginx را بایپاس میکنند.
