...
### ۴. تحویل کار Mirror کانال ریموت

میرور رسانه Telegram در کانال ریموت از همین ورکر برای کار سنگین رسانه استفاده می‌کند و اعمال محدودیت اندازه، بررسی ویدیو و بارگذاری در باکت را به ورکر می‌سپارد.

- **ارسال کار:** `POST /mirror-media` کار را با یک secret اختصاصی هر کار می‌پذیرد.
- **دریافت بایت‌ها:** ورکر بایت‌های رمزگشایی شده منبع را از مسیر محافظت شده با secret یعنی `GET /api/remote-channel/blob/:jobId` روی سرور می‌گیرد.
- **callback نتیجه:** `POST /api/remote-channel/webhook/mirror-done` فایل‌ها را متصل کرده، به‌روزرسانی SSE را منتشر می‌کند و تحویل‌های تکراری را حذف می‌کند.
- **Fallback:** هنگامی که یک ورکر ریموت در دسترس نباشد، سرور از مسیر inline محلی با تایمر timeout به عنوان fallback استفاده می‌کند. همچنین، رسانه میرور شده از fallback به دیسک محلی استفاده می‌کند تا هیچ رسانه‌ای از دست نرود. ویدیوهای میرور شده همچنان وارد pipeline عادی ترنسکد می‌شوند.
