...
| خطای CORS در مرورگر هنگام آپلود به باکت Railway | با `STORAGE_AUTO_CORS=true` (تنظیم شده در `.railway/railway.ts`) سرور خودش این کار را میکند — لاگ سرور را برای عبارت `Bucket CORS policy applied` / `already configured` بررسی کنید. اگر هشدار پیکربندی خودکار دیدید، دستی اعمال کنید: `npm --prefix server run storage:cors -- --origin https://your-app.up.railway.app`. |
| خطای `Failed to notify callback` در لاگ ورکر با پورت خالی (`...internal:/api/...`) | متغیر `PORT` را در `WEBHOOK_URL` قالب بندی نکنید — مقدار آن هنگام اجرای کانتینر تزریق میشود و هیچ ارجاع IaC نمیتواند آن را بگیرد. `WEBHOOK_URL` را خالی بگذارید تا سرور آن را از `RAILWAY_PRIVATE_DOMAIN` و `PORT` بسازد (`server/lib/webhookUrl.js`). |
| خطای `Failed to notify callback` در لاگ ورکر (رد اتصال) | برنامه باید ترافیک IPv6 شبکه خصوصی را بپذیرد: مقدار `BIND_ADDRESS=::` (دوگانه، dual-stack) را در Railway تنظیم کنید. همچنین مطمئن شوید هر دو سرویس دیپلوی مجدد را کامل کرده اند و برنامه روی هاست و پورت `WORKER_URL`/`WEBHOOK_URL` در دسترس است. |
