...
Songbird برای پیام‌های زنده، حضور و نشانگرهای تایپ از WebSocket پایدار استفاده می‌کند و از Server-Sent Events (SSE) به‌عنوان جریان جایگزین بهره می‌برد. یک کاربر تا زمانی که هر اتصال بی‌درنگ باز است آنلاین نمایش داده می‌شود و با بسته شدن آخرین اتصال آفلاین می‌شود. اگر پیام‌ها تنها پس از یک تازه‌سازی ظاهر می‌شوند، احتمالاً جریان SSE توسط Nginx بافر می‌شود.
