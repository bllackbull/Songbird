...
- 🎨 Admin panel redesigned to match the main app design language
- 📱 Mobile-friendly admin panel with full-page menu layout and card tables
- 🔔 Dashboard animated gauges and stat counters
- 🔧 Env vars always override DB settings; locked keys shown as disabled in UI
- 🔗 Child settings grouped under master cards; entire card disabled when env-locked
- 🔄 Unified refresh flow across all admin tabs
- 💾 Backup/restore simplified to plain `.db` copy (dropped zip/password support)
- 🌐 Exposed `APP_DEBUG`, chat client tuning, and remote proxy URLs in settings
- 🔒 API rate limiter mounted before routes; fixed spoofable loopback check
- 🔧 Bug fixes
